-- ========================================
-- YooNet Quest System - Complete Database Setup with Skill Assessment
-- ========================================
-- This file contains all database schema and updates needed for the YooNet Quest System
-- Execute this file on a fresh database to set up the complete skill assessment system
-- 
-- Version: 3.0 (October 2025)
-- Changes: Comprehensive skill assessment system with tier-based progression
--          Replaced XP system with skill-specific point accumulation
--          Added performance modifiers and skill level tracking
--
-- Usage: mysql -u root -p your_database_name < complete_database_setup_v3.sql
-- ========================================
...
-- 1. Users table (updated for skill progression)
...
-- 3. Comprehensive Skills table with tier points
CREATE TABLE IF NOT EXISTS `comprehensive_skills` (
  ...
  CONSTRAINT `comprehensive_skills_ibfk_1` FOREIGN KEY (`category_id`) REFERENCES `skill_categories` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 4. User Skill Progress table (replaces total_xp system)
CREATE TABLE IF NOT EXISTS `user_skill_progress` (
  ...
  CONSTRAINT `user_skill_progress_ibfk_1` FOREIGN KEY (`employee_id`) REFERENCES `users` (`employee_id`) ON DELETE CASCADE,
  CONSTRAINT `user_skill_progress_ibfk_2` FOREIGN KEY (`skill_id`) REFERENCES `comprehensive_skills` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- 7. Quests table (simplified for skill assessment)
CREATE TABLE IF NOT EXISTS `quests` (
  ...
  PRIMARY KEY (`id`),
  KEY `created_by` (`created_by`),
  CONSTRAINT `quests_ibfk_2` FOREIGN KEY (`created_by`) REFERENCES `users` (`employee_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 8. Quest Skills table (skills required/taught in quests)
...
-- 9. User Quests table (assignment tracking)
...
CREATE TABLE IF NOT EXISTS `quest_submissions` (
  ...
  CONSTRAINT `quest_submissions_ibfk_3` FOREIGN KEY (`reviewed_by`) REFERENCES `users` (`employee_id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 11. Skill Assessments table (per-skill grading)
CREATE TABLE IF NOT EXISTS `skill_assessments` (
  ...
  CONSTRAINT `skill_assessments_ibfk_1` FOREIGN KEY (`submission_id`) REFERENCES `quest_submissions` (`id`) ON DELETE CASCADE,
  CONSTRAINT `skill_assessments_ibfk_2` FOREIGN KEY (`skill_id`) REFERENCES `comprehensive_skills` (`id`) ON DELETE CASCADE,
  CONSTRAINT `skill_assessments_ibfk_3` FOREIGN KEY (`assessed_by`) REFERENCES `users` (`employee_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 12. Skill Points History table (track all skill point changes)
CREATE TABLE IF NOT EXISTS `skill_points_history` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `employee_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  `skill_id` int(11) NOT NULL,
  ...
  KEY `employee_id` (`employee_id`),
  KEY `skill_id` (`skill_id`),
  CONSTRAINT `skill_points_history_ibfk_1` FOREIGN KEY (`employee_id`) REFERENCES `users` (`employee_id`) ON DELETE CASCADE,
  CONSTRAINT `skill_points_history_ibfk_2` FOREIGN KEY (`skill_id`) REFERENCES `comprehensive_skills` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 13. User Achievements table (skill-based achievements)
CREATE TABLE IF NOT EXISTS `user_achievements` (
  ...
  CONSTRAINT `user_achievements_ibfk_2` FOREIGN KEY (`skill_id`) REFERENCES `comprehensive_skills` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- ========================================
-- SECTION 2: INSERT SKILL CATEGORIES
-- ========================================

INSERT IGNORE INTO `skill_categories` (`id`, `category_name`, `description`, `icon`, `color`) VALUES
...
-- ========================================
-- SECTION 3: INSERT COMPREHENSIVE SKILLS
-- ========================================
...
INSERT IGNORE INTO `users` (`employee_id`, `email`, `password`, `full_name`, `role`) VALUES
...
INSERT IGNORE INTO `quests` (`id`, `title`, `description`, `status`, `created_by`, `quest_assignment_type`) VALUES
(1, 'Build a Dockerized Python API', 'Create a REST API using Python and Flask, containerized with Docker', 'active', 'QL001', 'optional');

-- Sample quest skills
INSERT IGNORE INTO `quest_skills` (`quest_id`, `skill_id`, `tier_level`) VALUES
(1, 2, 2), -- Python Tier 2
(1, 27, 1), -- Docker Tier 1
(1, 43, 2); -- Git Version Control Tier 2

-- ========================================
-- SECTION 5: PERFORMANCE MODIFIERS REFERENCE
-- ========================================

-- Performance modifiers for skill assessments:
-- 'below_expectations' = -30% points
-- 'meets_expectations' = +0% points (base)
-- 'exceeds_expectations' = +25% points
-- 'exceptional' = +50% points

-- Skill Level Calculation:
-- Level 1 (Beginner): 0-99 points
-- Level 2 (Novice): 100-299 points
-- Level 3 (Competent): 300-699 points
-- Level 4 (Proficient): 700-1499 points
-- Level 5 (Advanced): 1500-2999 points
-- Level 6 (Expert): 3000-5999 points
-- Level 7 (Master): 6000+ points

-- Skill Stages:
-- LEARNING: Levels 1-3 (0-699 points)
-- APPLYING: Levels 4-5 (700-2999 points)
-- MASTERING: Levels 6-7 (3000-5999 points)
-- INNOVATING: Level 7+ (6000+ points)

-- Activity Status:
-- ACTIVE: Used within last 30 days
-- STALE: Used 30-90 days ago
-- RUSTY: Not used for 90+ days
...
-- Final status
SELECT 'YooNet Quest System v3.0 with Skill Assessment is ready for use!' as final_status;
