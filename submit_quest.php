...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $quest_id && $employee_id) {
    ...
    if ($submission_type === 'file') {
        if (!isset($_FILES['quest_file']) || $_FILES['quest_file']['error'] !== UPLOAD_ERR_OK) {
    ...
    if ($valid) {
        // Insert into quest_submissions (restore drive_link column)
        $stmt = $pdo->prepare("INSERT INTO quest_submissions (quest_id, employee_id, file_path, drive_link, text_content, comments, status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, 'submitted', NOW())");
        $stmt->execute([
            $quest_id,
            $employee_id,
            $file_path,
            $submission_type === 'link' ? $drive_link : null,
            $submission_type === 'text' ? $text : null,
            $comments
        ]);
