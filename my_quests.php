...
$my_quests = [];
try {
    $sql = "SELECT uq.*, q.title, q.description, q.due_date, q.quest_assignment_type, q.id as quest_id FROM user_quests uq JOIN quests q ON uq.quest_id = q.id WHERE uq.employee_id = ? AND uq.status IN ('in_progress','assigned','accepted','started','submitted') AND q.status = 'active' ORDER BY q.due_date ASC, q.created_at DESC";
